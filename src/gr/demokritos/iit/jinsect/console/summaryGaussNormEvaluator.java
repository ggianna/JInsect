/*
 * summaryGaussNormEvaluator.java
 *
 * Created on 04 June 2007, 15:21
 *
 */
 ...
 * @author ggianna
 */
public class summaryGaussNormEvaluator implements Runnable {
    // Constants
    ...
    // Variables
    Integer WordMin, WordMax, WordDist, CharMin, CharMax, CharDist, Threads, WeightMethod;
    String OutFile, SummaryDir, ModelDir, Do;
    boolean Silent, Progress, LowMem;
    Semaphore OutputSemaphore;
    Hashtable hModelCache = new Hashtable();
    Hashtable hNModelCache = new Hashtable();
    ...
    public static void main(String[] args) {
        // doRougeLikeEval();
        summaryGaussNormEvaluator sCur = new summaryGaussNormEvaluator(args);        
        sCur.run();
    }
    
    /** Provides command-line syntax information for the use of the main class function. */
    private static void printUsage() {
            System.err.println("Syntax:\nsummaryGaussNormEvaluator -do=(char|word|all) [-summaryDir=summaries/] [-modelDir=models/]"+
                    "[-wordMin=#] [-wordMax=#] [-wordDist=#] [-charMin=#] [-charMax=#] [-charDist=#] [-o=outFile] [-t=#]" + 
                    "[-s] [-use=o|d] [-progress] [-lowmem]");
     ...
     * mode.
     *@param bLowMem Use memory efficient (but much slower) method. Use in case of memory 
     * overflows.
     */
    public summaryGaussNormEvaluator(Semaphore sOutputSemaphore, String sDo,
        ...
        String sOutFile, String sSummaryDir, String sModelDir, boolean bSilent, int iWeightMethod,
            boolean bProgress, boolean bLowMem)
    {
        Do = sDo;
    ...
    /** Creates a new instance of summaryGaussNormEvaluator using command-line arguments.
     *@param args An array of strings corresponding to command line arguments.
     */
    public summaryGaussNormEvaluator(String[] args) {
        ...
        try {
            ...
            LowMem = gr.demokritos.iit.jinsect.utils.getSwitch(hSwitches, "lowmem", "FALSE").equals("TRUE");
    ...
    protected SimilarityArray calcSimilarityMeasures(CategorizedFileEntry cfeCur, List dsModelSet, 
        ...
        NGramGaussNormDocument ndNDoc1 = new NGramGaussNormDocument(CharacterNGramSize_Min, CharacterNGramSize_Max, Character_Dmax, CharacterNGramSize_Min, CharacterNGramSize_Max);
        ...
        while (iOtherIter.hasNext()) {
            ...
            NGramGaussNormDocument ndNDoc2 = null;
            ...
            if (bDoCharNGrams) {
                synchronized (hNModelCache) {
                    if (hNModelCache.containsKey(cfeOther.getFileName())) {
                        while ((ndNDoc2 = (NGramGaussNormDocument)hNModelCache.get(cfeOther.getFileName())) 
                            instanceof LoadingNGramDocument)
                            Thread.yield();
                    }
                    else {
                        ...
                        hNModelCache.put(cfeOther.getFileName(), new LoadingNGramDocument());

                        ndNDoc2.loadDataStringFromFile(cfeOther.getFileName());
                        if (LowMem && (hNModelCache.size() > 1))
                            hNModelCache.clear();
    ...
    /** Performs similarity measurement of a {@link CategorizedFileEntry}, given a model set. Uses
     * distribution-based method for assigning weights to n-gram graph edges.
     *@param cfeCur The current file to compare to models.
     *@param dsModelSet The input model set.
     *@param bOutput If true, output is verbose.
     *@param pOut The {@link PrintStream} to use for output.
    ...
    protected void doOptimizedEval(final Semaphore sSem, PrintStream pOverallResultsOutStream,
        ...
        while (iCatIter.hasNext()) {
            ...
            while (iIter.hasNext()) {
                ...
                Runnable r = new GaussCalcSimilRunner(WordNGramSize_Min, WordNGramSize_Max, Word_Dmax,
...
/** Utility class, used to perform Gauss-normalized method evaluation. */
class GaussCalcSimilRunner implements Runnable {
    
    // Variables
    int WordNGramSize_Min, WordNGramSize_Max, Word_Dmax;
    int CharacterNGramSize_Min, CharacterNGramSize_Max, Character_Dmax;
    CategorizedFileEntry CurEntry;
    Semaphore Sem;
    List CompareAgainst;
    boolean DoCharNGrams, DoWordNGrams;
    PrintStream OutStream;
    boolean Silent, Progress;
    summaryGaussNormEvaluator Caller;
    int WeightingMethod = -1;
    
    public GaussCalcSimilRunner(int iWordNGramSize_Min, int iWordNGramSize_Max, int iWord_Dmax,
            ...
            boolean bSilent, summaryGaussNormEvaluator seCaller, int iWeightingMethod,
            boolean bProgress) {
    ...
    public void run() {
        ...
        switch (WeightingMethod) {
            case summaryGaussNormEvaluator.USE_DISTRO_AVERAGE_AS_WEIGHT:
               ...
               sa = Caller.calcDistroSimilarityMeasures(CurEntry,
                    ...
                    Silent);
                break;
            case summaryGaussNormEvaluator.USE_OCCURENCES_AS_WEIGHT:
                // Just compare, ignoring identical summary from models
...
/** Dummy class to indicate loading of a document
 */
class LoadingNGramDocument extends NGramGaussNormDocument {
}
